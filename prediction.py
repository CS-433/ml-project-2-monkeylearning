...
# Parameters
num_patches = (TEST_IMAGE_SIZE + PATCH_SIZE - 1) // PATCH_SIZE
...
def preprocess_image(image_path):
    with Image.open(image_path) as img:
        img = img.resize((TEST_IMAGE_SIZE, TEST_IMAGE_SIZE))  # Resize to model input size
        img = np.array(img)
...
def main(test_images_dir=TEST_IMAGES_DIR):
    ...
    os.makedirs(PREDICTED_GROUNDTRUTH_DIR, exist_ok=True)

    # Recursively collect all test images from subfolders
    test_image_paths = sorted(glob(os.path.join(test_images_dir, '**/*.png'), recursive=True))
    ...
    for img_path in test_image_paths:
        ...
        output_image = np.zeros((TEST_IMAGE_SIZE, TEST_IMAGE_SIZE))
        weight_image = np.zeros((TEST_IMAGE_SIZE, TEST_IMAGE_SIZE))

        x_positions = get_patch_positions(TEST_IMAGE_SIZE, PATCH_SIZE, num_patches)
        y_positions = get_patch_positions(TEST_IMAGE_SIZE, PATCH_SIZE, num_patches)
        ...
        for y in y_positions:
            for x in x_positions:
                ...
                if y + PATCH_SIZE > TEST_IMAGE_SIZE or x + PATCH_SIZE > TEST_IMAGE_SIZE:
                    continue
                patch = img[y:y+PATCH_SIZE, x:x+PATCH_SIZE, :]
                patches.append(patch)
        ...
        pred_image.save(os.path.join(PREDICTED_GROUNDTRUTH_DIR, output_file_name))

    print(f"Predicted masks saved to {PREDICTED_GROUNDTRUTH_DIR}.")
