...
for i, (img_path, pred) in enumerate(zip(test_image_paths, predictions)):
    ...
    output_file_name = f'{subfolder_name}.png'  # Include unique identifier if necessary
    # Convert the prediction to uint8 format (binary mask with 255 for the foreground)
    pred_mask = (pred.squeeze() * 255).astype(np.uint8)  # Remove extra dimensions and convert
    # Resize to 608x608 using PIL
    pred_image = Image.fromarray(pred_mask).resize((608, 608), Image.NEAREST)  # Nearest neighbor for binary masks
